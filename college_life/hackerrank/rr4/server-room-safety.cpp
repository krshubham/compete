...
int main(){
	...
	for (int i = 0; i < n-1; ++i){
		auto x = towers[i];
		auto y = towers[i+1];
		maxl = max(maxl,x.second+x.first);
		if(maxl >= y.first){
			c++;
		}
	}
	...
	// cout<<c<<endl;
	if(c == n-1){
		a = 1; // left side makes them all fall
	}
	c = 0;
	lli minl = INT_MAX;
	for (int i = n-1; i >= 0; i--){
		...
		auto x = towers[i];
		auto y = towers[i-1];
		minl = min(minl,x.first - x.second);
		if(minl <= y.first){
			c++;
		}
	}
	if(c == n-1){
		b = 1; //right side makes them all fall
	}
	if(a == 1 && b == 1){
		cout<<"BOTH"<<endl;
	}
