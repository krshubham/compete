/**
* @author: krshubham
* @time: 23:09:07
**/
#pragma comment (linker, "/stack:20000000")
#pragma GCC optimize("Ofast,unroll-loops,no-stack-protector")
#pragma GCC target("sse,sse2,sse3,ssse3,sse4,popcnt,abm,mmx,avx,tune=native")
#include <bits/stdc++.h>
using namespace std;

#define bye return 0
#define pb push_back
#define mp make_pair
#define mod(n) (n) % 1000000007
#define e_val 2.718281828
#define stp(a,b) a.push(b)
#define all(a) a.begin(),a.end()
#define PI 3.1415926535897932384626433832795
#define rep(i,n) for( int i = 0; i < n; i++ )
#define endl '\n'

typedef long long int lli;
typedef long long ll;
typedef unsigned long long int ulli;
typedef unsigned long long ull;
typedef pair<lli,lli> plli;
typedef vector<lli> vlli;
typedef map<string,lli> mslli;
typedef map<lli,lli> mlli;
typedef unordered_map<lli,lli> umlli;
typedef vector<pair<lli,lli> > vplli;
inline vector<string> split(string str,string sep){char* cstr=const_cast<char*>(str.c_str());char* current;vector<string> arr;current=strtok(cstr,sep.c_str());while(current!=NULL){arr.push_back(current);current=strtok(NULL,sep.c_str());}return arr;}
inline bool isPrime(lli n){if (n <= 1){return false;}if (n <= 3)  {return true;}if (n%2 == 0 || n%3 == 0) {return false;}for (int i=5; i*i<=n; i=i+6){if (n%i == 0 || n%(i+2) == 0){return false;}}return true;}
inline lli power(lli x,lli y,lli p){int res=1;x=x%p;while(y>0){if(y&1)res=(res*x)%p;y = y>>1;x = (x*x) % p;}return res;}

// Courtesy: Geekforgeeks
bool isPower(lli x, lli y) { 
    lli res1 = log(y) / log(x); 
    long double res2 = log(y) / log(x);
    return (res1 == res2); 
} 

lli digit_sum(lli n){
    string s = to_string(n);
    lli sum = 0;
    for(auto& c:s){
        sum += (c-'0');
    }
    return sum;
}

lli digital_root(string num){
    if (num.compare("0") == 0) {
        return 0; 
    }
    int ans = 0; 
    for (int i=0; i< num.size(); i++) {
        ans = (ans + num[i]-'0') % 9; 
    }
    return (ans == 0)? 9 : (ans % 9); 
}
unordered_map<lli,lli> dp;
void findMinWays(lli num, lli d, lli _min, lli& depth){  
    if(dp[num] != 0){
        return dp[num];
    }
    if(depth > 12){
        for(auto it = dp.begin(); it != dp.end(); it++){
            cout<<it->first<<" "<<it->second<<endl;
        }
        return 0;
    }
    if(num == _min){
        return 0;
    }
    if(isPower(num,10LL)){
        return 1;
    }
    depth++;
    int waysWithDs = 2 + findMinWays(digit_sum(num+d),d,_min,depth);
    int waysWithSum = 1+findMinWays(digit_sum(num),d,_min,depth);
    return dp[num] = min(waysWithSum,waysWithDs);
}



int main(){
    ios_base::sync_with_stdio(0);
    cin.tie(nullptr);
    lli t,n,a,b,c,d,e,f,x,y;
    cin>>t;
    while(t--){
        dp.clear();
        cin>>n>>d;
        lli _ans = digital_root(to_string(n));
        x = n;
        for(int i = 0; i < 10; i++){
            _ans = min(_ans,digital_root(to_string(x+d)));
            x += d;
        }
        lli depth = 0;
        cout<<findMinWays(n,d,_ans,depth)<<endl;
    }
    bye;
}