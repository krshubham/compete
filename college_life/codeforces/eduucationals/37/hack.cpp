...
int main(){
    ...
    int maxx=v1[0],minn=v1[0];
    int curr=1,sp=0;
    int prev=-1;
    bool fl=true;
    while(sp<s.length() and curr<v1.size())
    {
        while(s[sp]!='0' and curr<v1.size())
        {
            maxx=max(maxx,v1[curr]);
            minn=min(minn,v1[curr]);
            curr++;sp++;
        }
        if(v1[curr]<prev)
        {
        ...
        {
            prev=v1[curr];
            vp.push_back({maxx,minn});
        }
        cout<<" curr "<<curr<<" sp "<<sp<<endl;
        curr++;sp++;
    }
    if(fl)
    {
        int curr_maxx=vp[0].first;
        int curr_minn=vp[0].second;
        for(int i=1;i<vp.size();i++)
        {
            if(curr_maxx<vp[i].second)
            {
                curr_maxx=vp[i].first;
            }
            else
            {
                fl=false;
