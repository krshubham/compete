...
int whichBin(float data,
  float bin_maxes[],
  int bin_count,
  float min_meas);

void print(float bin_maxes[],
  int bin_counts[],
  int bin_count,
  float min_meas);
...
int main(int argc, char* argv[]) {
  ...
  getInput(&bin_count, &min_meas, &max_meas, &data_count,
  ...
  makeBins(bin_maxes,loc_bin_cts,min_meas,max_meas,bin_count,my_rank,comm);
  genData(local_data,local_data_count,data_count,min_meas,max_meas,my_rank,comm);
  findBins(bin_counts,local_data,loc_bin_cts,local_data_count,bin_maxes,bin_count,min_meas,comm);
...
void findBins(
 ...
 MPI_Comm comm
  ...
  for(i = 0; i < local_data_count; i++) {
    bin = whichBin(local_data[i],bin_maxes,bin_count,min_meas);  
    loc_bin_cts[bin]++;
  }
}

int whichBin(float data, float bin_maxes[], int bin_count,
 float min_meas) {
...
void getInput(
 ...
 MPI_Comm comm) {
  ...
  if(my_rank == 0) {
    printf("Enter the number of bins(Integer)");
    scanf("%d",bin_count_p);
    printf("Minimum float value: ");
    scanf("%f",min_meas_p);
    printf("Maximum float value: ");
    scanf("%f",max_meas_p);
    ...
    printf("Enter the number of values(Integer): ");
